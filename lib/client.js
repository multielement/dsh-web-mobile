...
exports.sniffImageSize = sniffImageSize;
 ...
 */
function sniffImageSize(head) {
    const be16 = (i) => ((head[i] ?? 0) << 8) | (head[i + 1] ?? 0);
    const be32 = (i) => (((head[i] ?? 0) << 24) | ((head[i + 1] ?? 0) << 16) | ((head[i + 2] ?? 0) << 8) | (head[i + 3] ?? 0)) >>> 0;
    const le16 = (i) => (head[i] ?? 0) | ((head[i + 1] ?? 0) << 8);
...
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.installImageIntakeBridge = installImageIntakeBridge;
...
function findHostFileInput(root = document) {
    const scope = root.querySelector('[data-slot="conversation.input"]')
        ?? root.querySelector('[class*="_card"]:has(textarea, [data-composer-input])')
        ?? root;
...
async function normalizeBatch(files) {
...
function writeBack(input, files) {
    try {
 ...
 */
function installImageIntakeBridge() {
    const onChange = (event) => {
        const target = event.target;
        if (!(target instanceof HTMLInputElement))
            return;
        if (target.type !== 'file' || target.dataset[OWN_INPUT_FLAG] === '1')
            return;
...
function MobileImagePicker({ t }) {
    const [needsOwnButton, setNeedsOwnButton] = (0, react_1.useState)(false);
    (0, react_1.useEffect)(() => installImageIntakeBridge(), []);
    (0, react_1.useEffect)(() => {
...
exports.MISC_CSS = `@media (max-width: 1023px) and (pointer: coarse) {
  ...
  /* Fallback-only paperclip (pre-0.1.5 hosts that ship no file input). On
     0.1.5+ the companion renders nothing: the host already draws its own
     paperclip in the round _add chip, and adding a second one crowded it. */
