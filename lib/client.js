...
function markGestureConsumed(target, windowMs, upTo) {
    ...
    if (consumed.size >= MAX_CONSUMED_ENTRIES)
        sweepExpired(performance.now());
 ...
 */
function MobileNavToggle({ toggleSidebar, openHostFiles, t }) {
 ...
 * Hidden entirely on wide screens (CSS media query).
 */
function MobileDrawerFooter({ useSessions, downloadSessionLog, toggleSidebar, openHostFiles, t }) {
    ...
    return ((0, jsx_runtime_1.jsxs)("div", { "data-mobile-nav": "drawer-actions", children: [(0, jsx_runtime_1.jsxs)("button", { type: "button", "data-mobile-nav": "explorer", "aria-label": t('files'), title: t('files'), onClick: openExplorer, children: [(0, jsx_runtime_1.jsx)(dsh_client_ui_primitives_1.IconPanelLeftOutline16, { size: 14 }), (0, jsx_runtime_1.jsx)("span", { children: t('files') })] }), (0, jsx_runtime_1.jsxs)("button", { type: "button", "data-mobile-nav": "session-log", "aria-label": t('sessionLog'), title: t('sessionLog'), disabled: sessionId === undefined, onClick: () => {
                ...
                }, children: [(0, jsx_runtime_1.jsx)(dsh_client_ui_primitives_1.IconDownloadOutline16, { size: 14 }), (0, jsx_runtime_1.jsx)("span", { children: t('sessionLog') })] }), (0, jsx_runtime_1.jsxs)("button", { type: "button", "data-mobile-nav": "tokens-total", "aria-label": tokensTitle, title: tokensTitle, onClick: refreshTokens, children: [(0, jsx_runtime_1.jsxs)("span", { className: "tokens-main", children: [(0, jsx_runtime_1.jsx)(dsh_client_ui_primitives_1.IconSparkle16, { size: 14 }), (0, jsx_runtime_1.jsx)("span", { children: t('tokensTotal') }), (0, jsx_runtime_1.jsx)("span", { className: "tokens-value", children: tokensDisplay })] }), tokens.status === 'loaded' && ((0, jsx_runtime_1.jsxs)("span", { className: "tokens-breakdown", children: [(0, jsx_runtime_1.jsx)("span", { children: `${t('tokInput')} ${formatCompact(tokens.breakdown.input)}` }), (0, jsx_runtime_1.jsx)("span", { children: `${t('tokOutput')} ${formatCompact(tokens.breakdown.output)}` }), (0, jsx_runtime_1.jsx)("span", { children: `${t('tokCacheRead')} ${formatCompact(tokens.breakdown.cacheRead)}` }), (0, jsx_runtime_1.jsx)("span", { children: `${t('tokCacheWrite')} ${formatCompact(tokens.breakdown.cacheWrite)}` }), tokens.breakdown.reasoning > 0 && ((0, jsx_runtime_1.jsx)("span", { children: `${t('tokReasoning')} ${formatCompact(tokens.breakdown.reasoning)}` }))] }))] })] }));
}
};
__modules["core/image-intake.js"] = function (require, module, exports) {
"use strict";
/**
...
exports.canStageImageDrop = canStageImageDrop;
exports.stageImagesAsDrop = stageImagesAsDrop;
 ...
 * surface fail).
 */
function canStageImageDrop(support) {
    return support.hasDataTransfer && (support.hasDragEventConstructor || support.hasLegacyDragEvent);
}
/**
 ...
 */
function stageImagesAsDrop(files, deps) {
    const images = files.filter(isImageFile);
    if (images.length === 0)
        return null;
    const makeDataTransfer = deps.makeDataTransfer;
    if (makeDataTransfer === undefined)
        return null;
    let transfer;
    try {
 ...
 */
function buildDropEvent(deps, transfer) {
...
__modules["core/image-normalize.js"] = function (require, module, exports) {
"use strict";
/**
 ...
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.ImageDecodeError = exports.DEFAULT_IMAGE_LIMITS = exports.HOST_IMAGE_TYPES = void 0;
...
exports.HOST_IMAGE_TYPES = ['image/png', 'image/jpeg', 'image/webp', 'image/gif'];
/** Conservative defaults mirroring dsh-attachment-local's shipped config. */
exports.DEFAULT_IMAGE_LIMITS = Object.freeze({
    maxBytes: 20971520,
    maxDimension: 8192,
    maxPixels: 64000000,
});
...
function fitWithin(width, height, maxDimension, maxPixels) {
    ...
    if (longest > maxDimension)
        scale = Math.min(scale, maxDimension / longest);
 ...
 */
async function normalizeImage(file, deps = {}) {
    const limits = deps.limits ?? exports.DEFAULT_IMAGE_LIMITS;
    let type = file.type;
    ...
    if (deps.readHead !== undefined && (type === '' || type === 'image/jpg' || !exports.HOST_IMAGE_TYPES.includes(type))) {
        try {
    ...
    const accepted = exports.HOST_IMAGE_TYPES.includes(type);
    ...
    try {
        ...
        if (ctx === null)
            return { file, changed: false, reason: 'no-2d-context' };
        ...
        if (best === null)
            return { file, changed: false, reason: 'encode-failed' };
...
__modules["components/MobileImagePicker.js"] = function (require, module, exports) {
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.MobileImagePicker = MobileImagePicker;
const jsx_runtime_1 = require("react/jsx-runtime");
const dsh_client_ui_primitives_1 = require("@deepseek-ai/dsh-client-ui-primitives");
const image_intake_ts_1 = require("./core/image-intake.js");
const image_normalize_ts_1 = require("./core/image-normalize.js");
/** Engine capability probe, evaluated once at load (client-only module). */
const INTAKE_SUPPORTED = (0, image_intake_ts_1.canStageImageDrop)({
...
function makeDragEvent(type, init) {
    try {
...
function makeLegacyDropEvent(transfer) {
    try {
 ...
 */
function findHostFileInput() {
    const scope = document.querySelector('[data-slot="conversation.input"]')
        ?? document.querySelector('[class*="_card"]:has(textarea, [data-composer-input])')
        ?? document;
    const inputs = scope.querySelectorAll('input[type="file"]');
    for (const input of inputs) {
        if (input.dataset[OWN_INPUT_FLAG] === '1')
            continue;
        return input;
    }
    return null;
 ...
 */
function feedHostInput(input, files) {
    try {
...
function MobileImagePicker({ t }) {
    ...
    const pickImages = async () => {
        ...
        const normalized = await Promise.all(picked.map(async (file) => {
            try {
                const result = await (0, image_normalize_ts_1.normalizeImage)(file, NORMALIZE_DEPS);
                return result.file;
            }
            catch {
                return file;
        ...
        if (hostInput !== null && feedHostInput(hostInput, normalized))
            return;
        // Fallback for host generations without a file input: stage as one drop.
        const staged = (0, image_intake_ts_1.stageImagesAsDrop)(normalized, { makeDataTransfer, makeDragEvent, makeLegacyDropEvent });
        if (staged !== null)
            document.dispatchEvent(staged.event);
    };
    return ((0, jsx_runtime_1.jsx)("button", { type: "button", "data-mobile-nav": "image-picker", "aria-label": t('uploadImage'), title: t('uploadImage'), onClick: () => { void pickImages(); }, children: (0, jsx_runtime_1.jsx)(dsh_client_ui_primitives_1.IconPaperclipOutline16, { size: 14 }) }));
}
};
...
function installDebugBadge(ctx) {
    ctx.effect(() => {
        ...
        const pushError = (line) => {
            errors.push(line);
            if (errors.length > ERROR_HISTORY_LIMIT)
                errors.shift();
        };
        const onError = (event) => pushError(`ERR ${event.message.slice(0, 120)}`);
        const onRejection = (event) => pushError(`REJ ${String(event.reason).slice(0, 120)}`);
...
exports.en = {
    ...
    'tokInput': 'In',
    'tokOutput': 'Out',
    'tokCacheRead': 'Cache R',
    'tokCacheWrite': 'Cache W',
    'tokReasoning': 'Think',
...
const MobileImagePicker_tsx_1 = require("./components/MobileImagePicker.js");
...
function apply(ctx) {
    ...
    ctx.slots.inject('conversation.input.left', () => ctx.slots.register({
        ...
        locale: locales_ts_1.NS,
        inject: () => ({}),
    }, MobileImagePicker_tsx_1.MobileImagePicker));
