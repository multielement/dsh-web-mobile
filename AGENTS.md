...
- **DSHA 0.1.5 适配补丁（2026-09-16 合入）**：侧栏 toggle `top: 18px`（与页首控制行中心 32px 对齐，release「按钮与左侧入口的中心均为 32px」）；composer 输入 13px 基准 + `--dsh-web-mobile-font-delta`（layout.css.ts，`html:not([data-mobile-nav-ios])` 门控——iOS 16px #45 floor 拥有该路径，勿混）；compat.css.ts 右侧面板 overlay（`[data-rightbar-col]` fixed 全屏 z-index 35、`[data-sidebar-right-open]` 才接事件、标签关闭按钮除外 32px 最小高；0.1.5 前宿主标记缺席整体空转）；package.json inject 含 `@deepseek-ai/dsh-client-ui-sidebar-right`（peer `>=0.1.5-rc.0 <0.2.0` 且 optional，老宿主安装不受影响）。其余 DSHA 私有项未合入：预设入口锚点（.dsha-preset-header-anchor 族）、composer `_modes` 弹性族、删除确认底部弹层化、backdrop 动画、icon-label 折叠族。
