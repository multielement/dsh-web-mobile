...
- `src/client/index.tsx` injects `['slots', 'layout', 'locale', 'sessionLogDownload', 'sessions', 'workspaces']`. Its `apply()` registers locale dictionaries, injects one `<style data-plugin>` tag, installs effects, and registers exactly three slots:
  - `conversation.session.header.actions` → `MobileNavToggle` (`order: 10`): drawer toggle + Files button. Files prefers the host-native right panel (`ctx.sidebarRight.openTab('files')`, 0.1.5+) and falls back to the dsh-web-ui explorer marker on older hosts.
...
- **DSHA 0.1.5 适配补丁（2026-09-16 合入）**：侧栏 toggle `top: 18px`（与页首控制行中心 32px 对齐，release「按钮与左侧入口的中心均为 32px」）；composer 输入 13px 基准 + `--dsh-web-mobile-font-delta`（layout.css.ts，`html:not([data-mobile-nav-ios])` 门控——iOS 16px #45 floor 拥有该路径，勿混）；compat.css.ts 右侧面板 overlay（`[data-rightbar-col]` fixed 全屏 z-index 35、`[data-sidebar-right-open]` 才接事件、标签关闭按钮除外 32px 最小高；0.1.5 前宿主标记缺席整体空转）；package.json inject 含 `@deepseek-ai/dsh-client-ui-sidebar-right`（peer `>=0.1.5-rc.0 <0.2.0` 且 optional，老宿主安装不受影响）。其余 DSHA 私有项未合入：删除确认底部弹层化（本仓库居中 420px 卡片为刻意设计）、icon-label 折叠族（本地 header 拥挤保护已覆盖等价行为）。**2026-09-18 追加**：预设入口锚点族已并入——`.dsha-preset-header-anchor`（layout 手机块 order 1/width max-content/max-width min(44vw,220px)/margin-left auto + 内层 `[data-dsha-agent-preset="header"]` inline-flex 44px，字号 12px 基准 + delta；compat 宽屏触控块 order 90/靠右，不含 DSHA 的文件入口常显条——本仓库宽屏触控保持桌面布局，仅会话删除三件套豁免）；页首弹层定位加 `:not([class*="_menuAnchor"])`（子串 `_menu` 命中锚点容器，DSHA 修正）；composer 权限/计划控件收缩规则由 `> :nth-child(2)` 改为 `> :is(:nth-child(2), [class*="_modes"])`（DSHA 0.1.5 该容器不在第 2 槽；`_modes` 在本 lane 唯一，并集是严格超集，上游已验证的槽位路径原样保留）。已核实「backdrop 动画」「icon-label 折叠」在本地 2.5.x 已有等价实现（base.css.ts 有 fade/sheet-in 动画，reduced-motion 才 `animation:none`），此前 diff 误报。**2026-09-18 后半（0.1.5-rc.2 定向适配）**：① Files 入口改为优先调用宿主原生右侧面板 `ctx.sidebarRight.openTab('files')`（0.1.5 起；`index.tsx` 的 `openHostFiles` 在点击时探测并 try/catch，服务缺席/异常返回 false → 组件回退旧 `data-aionui-explorer-open` 路径），`tsconfig.client.json` 加 `@deepseek-ai/dsh-client-ui-sidebar-right/client` 路径映射 + 类型增强 import；② viewport 改为无缩放 token 的 `width=device-width, initial-scale=1, viewport-fit=cover, interactive-widget=resizes-content`（去掉 2026-09-16 安卓缩放锁，与 DSHA 0.1.5 对齐，探针 A8-A10 同步）。DSHA 精确 pin（`0.1.5-rc.2`）与 DSHA 壳专用事件（`dsha-session-open` / `[data-dsha-session-select]`）按用户取舍**不并入**（保持本仓库多宿主范围兼容与独立于 DSHA APK）。
...
- **iOS 一输入就放大（#45）已按机制修**（完整取证/已否决路线/未验证后续项 → `docs/maintenance/pitfalls.md` §iOS zoom）：根/抽屉 `touch-action` 必须含 `pinch-zoom`（沿祖先链交集，漏一层授权就被抵消）；`gesturestart` 一律不 preventDefault；16px 下限走 `html[data-mobile-nav-ios]`（misc.css，盖 textarea / `[contenteditable]:not([contenteditable="false"])` / 文本类 input + composer 三件套同字号；`select` 故意不改）；引擎判定=纯函数 `detectIosWebKit`（先 CSS.supports 特征探针再 UA，iPadOS 13+ 发桌面 UA 靠 maxTouchPoints）。viewport meta 所有权（重申 `width=device-width, initial-scale=1, viewport-fit=cover, interactive-widget=resizes-content`）已合并 cb16329——**2026-09-18 所有者决定去掉缩放锁、与 DSHA 0.1.5 对齐**（pinch/双击/无障碍缩放交还引擎、软键盘收缩内容区；取代 2026-09-16 的安卓缩放钉死）；root touch-action 的 `pinch-zoom` 保留。探针 `scripts/cdp-zoom-probe.mjs`（21 断言）。
