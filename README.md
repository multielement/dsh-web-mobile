...
- **缩放交还引擎**：viewport 不锁缩放，捏合、双击、无障碍文本缩放都可用；软键盘弹出时收缩内容区而非遮挡输入框（与 DSHA 0.1.5 对齐）
...
> **最新版本 v2.5.7-mod** 修好了两处会让多模态能力形同虚设的问题：① 移动端上传的图片此前无法真正送进模型——DSHA 0.1.5 的附件收件管线不认插件合成的 drop 事件，且相册给出的非标准 MIME（`image/jpg`）、空 MIME、HEIC 以及超尺寸原图都会被宿主准入白名单拒掉；② 插件自画的回形针与宿主自带的那颗并排，视觉上重复拥挤。现在改为**拦宿主 input 的 change 做归一化**，宿主自己的按钮即唯一入口。详见「上传图片（视图功能）」与「更新内容 → v2.5.7」。
...
宿主 0.1.5 的输入框左侧工具栏**自带**一个回形针按钮（与命令加号并排，同在圆形的 `_add` 芯片里），背后是一个隐藏的 `<input type="file" multiple>`。本插件**不再另加按钮**，而是**捕获阶段拦下这个 input 的 `change`**：把选中的文件**先在设备上归一化**，写回去，再重新派发 `change`——于是宿主自己的 `onPickFiles` / `intakeFiles` 拿到的已经是归一化过的批次，缩略图预览、上传、消息内嵌图、模型视觉内容块全部复用宿主原生链路。

> 早期实现是自画一个回形针按钮 + 合成 drop 事件。两个问题：① 与宿主的按钮并排，视觉上重复、挤在一起；② 合成 drop 在 0.1.5 的原生收件管线里不生效，选完图毫无反应。现版本两处都已修正。
...
归一化做的事：按文件头字节嗅探真实容器（JPEG / PNG / GIF / WebP / HEIC / AVIF / BMP），把 `image/jpg` 与空 MIME 纠正回标准类型；超出尺寸 / 像素上限的先降采样，再沿质量阶梯（0.92 → 0.45）重编码到 20MB 以内。普通的小尺寸合规图片直接透传（尺寸归一化交给宿主自带的 sharp 管线，不浪费手机 CPU）；解码失败时回退原始文件，绝不丢图。

旧宿主（无自带回形针按钮）仍会回退到插件自画的按钮。
...
- **上传按钮重复且拥挤**：插件自画的回形针与宿主 0.1.5 自带的回形针并排渲染，两个图标挤在同一行（用户视角是“挤在色差圆圈边缘”）。现改为**不再另加按钮**，只保留对宿主 input 的 change 拦截，宿主自己的回形针成为唯一入口；旧宿主才回退到插件按钮。
...
- Files 入口优先打开宿主原生右侧面板（0.1.5 起），旧宿主无该服务时自动回退 dsh-web-ui 文件树
- viewport 改为与 DSHA 0.1.5 对齐：不锁缩放，软键盘弹出时收缩内容区而非遮挡输入框
